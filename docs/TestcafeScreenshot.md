...
Overall point is that, inside your test, you have no access to the configuration of the browser currently run. Examples where this would be beneficial
...
- build a subdirectory for screenshots for each running browser
- read screenshots from directory

Infos from the testcafe configuration to current live data - all about opts, browser and current test are parsed and evaluated, held in sync by testcafe.

But not officially accessible for the end user, meaning effort and duplication.
